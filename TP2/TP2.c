...
Date2 *chargement(char *nom, int *n){
    ...
    fscanf(f,"%d",n);
    Date2 *t = (Date2 *)malloc(*n * sizeof(Date2));
    for(int k =0;k< *n;k++){
        fscanf(f,"%hd%hd%hd%hd", &(t+k)->n, &(t+k)->j,&(t+k)->m,&(t+k)->a);
        
        (t + k)->h  = (Heure *)malloc(((t+k)->n) * sizeof(Heure));
        for(int i=0; i < (t+k)->n; i++){

            fscanf(f,"%hd %hd %lf", &((t+k)->h + i)->h, &((t+k)->h+i)->m, &(((t+k)->h)+i)->s);
        }
    } return t;
}
...
int est_trie(Date2 *d,int n){
    ...
    for(int k = 0;k<n-1;k++){
        ...
        d2.m = (d+k+1)->m;
        d2.j = (d+k+1)->j;
        int a = difference_date(d1,d2);
        if(a == -1){
            
            return 0;
        }
        else if( difference_date(d1,d2) == 0 && difference_heure(&h3,*(d+k)->h ,*(d+k+1)->h) == 0){
            return 0;
        }
    }
    return 1;
